...
class Finances:
    ...
    def read_debts(self,month=None):
        ...
        model = GS.Debts
        d_list = []
        resp = self.gsheets.get(model.table,majorDimension='ROWS')
        if 'values' in resp:
    ...
    def get_bank_statement(self,date=None):
        ...
        date = close_day
        cd = self.get_close_day()
        transactions = Nubank().get_transactions()
        ...
        for t in transactions['edges']:
            ...
            if date.date() >= close_day.date():
                    ref = date.month if date.day < cd else date.month + 1
                    bs_list.extend(Debts.nuconta_debts(t,ref))
